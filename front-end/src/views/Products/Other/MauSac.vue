<template>
  <div>
    <!-- Breadcrumb -->
    <Breadcrumb breadcrumb="Màu sắc" />

    <div class="mt-4">
      <h4 class="text-gray-600">Màu sắc</h4>
        ...
        <div class="w-full max-w-4xl overflow-hidden bg-white border rounded-md shadow-md">
          <form>
            <div class="flex items-center justify-between px-5 py-3 text-gray-700 border-b">
              <h3 class="text-sm">Thêm Màu sắc</h3>
              <button>
    ...
    <div class="mt-8">
      <h4 class="text-gray-600">Danh sách Màu sắc</h4>
      <div class="mt-4">
        <table class="w-full bg-white rounded-md shadow-md">
          ...
          <tbody>
            <tr class="border-t text-center" v-for="color in colors" :key="color.id">
              <td class="px-4 py-2">{{ color.id }}</td>
              <td class="px-4 py-2">{{ color.code }}</td>
...
const colors = ref([
  { id: "C001", code: "RED", name: "Đỏ" },
  { id: "C002", code: "BLUE", name: "Xanh dương" }
]);
</script>
