<template>
  <div>
    <!-- Breadcrumb -->
    <Breadcrumb breadcrumb="PIN" />

    <div class="mt-4">
      <h4 class="text-gray-600">PIN</h4>
        ...
        <div class="w-full max-w-4xl overflow-hidden bg-white border rounded-md shadow-md">
          <form>
            ...
            <div class="p-5 text-gray-700 bg-gray-200 border-b grid grid-cols-3 gap-4">
              ...
              <div>
                <label class="text-xs">Loại Pin</label>
                <input type="text" class="w-full px-4 py-2 mt-2 border rounded-md" />
              </div>
              <div>
                <label class="text-xs">Dung lượng Pin</label>
                <input type="text" class="w-full px-4 py-2 mt-2 border rounded-md" />
              </div>
            </div>
    ...
    <div class="mt-8">
      <h4 class="text-gray-600">Danh sách PIN</h4>
      <div class="mt-4">
        <table class="w-full bg-white rounded-md shadow-md">
          <thead>
            <tr class="bg-gray-200 text-gray-700">
              ...
              <th class="px-4 py-2">Loại Pin</th>
              <th class="px-4 py-2">Dung lượng Pin</th>
              <th class="px-4 py-2">Hành động</th>
            </tr>
          </thead>
          <tbody>
            <tr class="border-t text-center" v-for="battery in batteries" :key="battery.id">
              <td class="px-4 py-2">{{ battery.id }}</td>
              <td class="px-4 py-2">{{ battery.code }}</td>
              <td class="px-4 py-2">{{ battery.type }}</td>
              <td class="px-4 py-2">{{ battery.capacity }}</td>
              <td class="px-4 py-2">
...
const batteries = ref([
  { id: "B001", code: "PIN001", type: "Li-Ion", capacity: "5000mAh" },
  { id: "B002", code: "PIN002", type: "Li-Po", capacity: "4500mAh" }
]);
</script>
