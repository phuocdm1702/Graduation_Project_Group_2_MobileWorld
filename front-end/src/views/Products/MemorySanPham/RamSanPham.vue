<template>
  <div>
    ...
    <div class="mt-8">
      <h4 class="text-gray-600">Danh sách RAM</h4>
      <div class="mt-4">
        <table class="w-full bg-white rounded-md shadow-md">
          ...
          <tbody>
          <tr class="border-t text-center" v-for="ram in rams" :key="ram.id">
            <td class="px-4 py-2">{{ ram.id }}</td>
            <td class="px-4 py-2">{{ ram.code }}</td>
            <td class="px-4 py-2">{{ ram.capacity }}</td>
            <td class="px-4 py-2">
...
const breadcrumbItems = computed(() => {
  ...
  return route.meta?.breadcrumb || ["RAM"]; // Mặc định nếu không có breadcrumb
});

const rams = ref([
  { id: "RAM001", code: "RAM-4GB", capacity: "4GB" },
  { id: "RAM002", code: "RAM-8GB", capacity: "8GB" }
]);
</script>