<template>
  <div>
        ...
        <div class="w-full max-w-4xl overflow-hidden bg-white border rounded-md shadow-md">
          <form>
            <div class="flex items-center justify-between px-5 py-3 text-gray-700 border-b">
              <h3 class="text-sm">Thêm Bộ nhớ trong</h3>
              <button>
            ...
            <div class="p-5 text-gray-700 bg-gray-200 border-b grid grid-cols-2 gap-4">
              ...
              <div>
                <label class="text-xs">Dung lượng bộ nhớ trong</label>
                <input type="text" class="w-full px-4 py-2 mt-2 border rounded-md" />
              </div>
            </div>
    ...
    <div class="mt-8">
      <h4 class="text-gray-600">Danh sách Bộ nhớ trong</h4>
      <div class="mt-4">
        <table class="w-full bg-white rounded-md shadow-md">
          ...
          <tr class="bg-gray-200 text-gray-700">
            ...
            <th class="px-4 py-2">Dung lượng bộ nhớ trong</th>
            <th class="px-4 py-2">Hành động</th>
          </tr>
          </thead>
          <tbody>
          <tr class="border-t text-center" v-for="storage in storages" :key="storage.id">
            <td class="px-4 py-2">{{ storage.id }}</td>
            <td class="px-4 py-2">{{ storage.code }}</td>
...
const breadcrumbItems = computed(() => {
  ...
  return route.meta?.breadcrumb || ["Bộ nhớ trong"]; // Mặc định nếu không có breadcrumb
});

const storages = ref([
  { id: "ST001", code: "ST001", capacity: "64GB" },
  { id: "ST002", code: "ST002", capacity: "128GB" },
  { id: "ST003", code: "ST003", capacity: "256GB" },
  { id: "ST004", code: "ST004", capacity: "512GB" },
  { id: "ST005", code: "ST005", capacity: "1TB" }
]);
</script>