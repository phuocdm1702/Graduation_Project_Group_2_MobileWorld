...
const breadcrumbItems = computed(() => {
  ...
  return route.meta?.breadcrumb || ["Quản Lý Imel", "Thêm Mới Imel"]; // Mặc định cho trang thêm mới
});
...
imel.value = { id: null, ma: '', imel: '' }; // Đặt lại dữ liệu mặc định
...
const handleSubmit = async () => {
  ...
  confirmAction('Bạn có chắc chắn muốn thêm Imel này?', async () => {
    await saveImel();
    router.push('/imel');
  });
};
