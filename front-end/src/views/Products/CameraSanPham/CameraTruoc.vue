<template>
  <div>
    <!-- Breadcrumb -->
    <Breadcrumb breadcrumb="Quản lý Camera Trước" />

    <div class="mt-4">
      <h4 class="text-gray-600">Quản lý Camera Trước</h4>
        ...
        <div class="w-full max-w-4xl overflow-hidden bg-white border rounded-md shadow-md">
          <form>
            <div class="flex items-center justify-between px-5 py-3 text-gray-700 border-b">
              <h3 class="text-sm">Thêm Camera Trước</h3>
              <button>
            ...
            <div class="p-5 text-gray-700 bg-gray-200 border-b grid grid-cols-3 gap-4">
              ...
              <div>
                <label class="text-xs">Tên Camera Trước</label>
                <input type="text" class="w-full px-4 py-2 mt-2 border rounded-md" />
              </div>
            </div>
    ...
    <div class="mt-8">
      <h4 class="text-gray-600">Danh sách Camera Trước</h4>
      <div class="mt-4">
        <table class="w-full bg-white rounded-md shadow-md">
          <thead>
            <tr class="bg-gray-200 text-gray-700">
              ...
              <th class="px-4 py-2">Tên Camera Trước</th>
              <th class="px-4 py-2">Hành động</th>
            </tr>
          </thead>
          <tbody>
            <tr class="border-t text-center" v-for="camera in cameras" :key="camera.id">
              <td class="px-4 py-2">{{ camera.id }}</td>
              <td class="px-4 py-2">{{ camera.code }}</td>
...
const cameras = ref([
  { id: "CM001", code: "CAM2001", name: "Camera Trước 8MP" },
  { id: "CM002", code: "CAM2002", name: "Camera Trước 12MP" }
]);
</script>
