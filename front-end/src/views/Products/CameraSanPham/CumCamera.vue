<template>
  <div>
        ...
        <div class="w-full max-w-4xl overflow-hidden bg-white border rounded-md shadow-md">
          <form>
            <div class="flex items-center justify-between px-5 py-3 text-gray-700 border-b">
              <h3 class="text-sm">Thêm Cụm Camera</h3>
              <button>
    ...
    <div class="mt-8">
      <h4 class="text-gray-600">Danh sách Cụm Camera</h4>
      <div class="mt-4">
        <table class="w-full bg-white rounded-md shadow-md">
          ...
          <tbody>
          <tr class="border-t text-center" v-for="cluster in cameraClusters" :key="cluster.id">
            <td class="px-4 py-2">{{ cluster.id }}</td>
            <td class="px-4 py-2">{{ cluster.code }}</td>
            <td class="px-4 py-2">
...
const breadcrumbItems = computed(() => {
  ...
  return route.meta?.breadcrumb || ["Cụm Camera"]; // Mặc định nếu không có breadcrumb
});

const cameraClusters = ref([
  { id: "CC001", code: "CAMC001" },
  { id: "CC002", code: "CAMC002" }
]);
</script>