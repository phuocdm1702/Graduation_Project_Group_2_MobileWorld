  ...
  <div class="overflow-x-auto mt-6 bg-white shadow-lg rounded-lg p-4">
    <table class="w-full min-w-max table-auto border-collapse">
      ...
      <tr class="bg-gray-100 text-gray-700 text-sm font-semibold">
        <th v-for="(column, index) in columns" :key="index" class="th-cell">
          {{ column }}
        </th>
      </tr>
      </thead>
      ...
      <tr v-for="(hoaDon, index) in dataTable" :key="hoaDon.id" class="text-gray-700 border-b hover:bg-gray-50">
        <td v-for="(column, colIndex) in columns" :key="colIndex" class="td-cell">
          {{ formatValue(column, hoaDon, index) }}
        </td>
      </tr>
      </tbody>
    </table>
...
const formatValue = (column, hoaDon, index) => {
  switch (column) {
    case 'STT':
      return index + 1;
    case 'Tổng tiền':
    case 'Sau Giảm giá':
      return `${(hoaDon[column.replace(' ', '')] || 0).toLocaleString()} VND`;
    case 'Ngày Tạo':
      return new Date(hoaDon.ngayTao).toLocaleDateString();
    case 'Trạng thái':
      return hoaDon.trangThai === 1 ? 'Hoàn thành' : 'Chờ xử lý';
    case 'Nhân viên':
      return hoaDon.idNhanVien?.ma || 'N/A';
    case 'Khách hàng':
      return hoaDon.idKhachHang?.ten || 'N/A';
    case 'Thao tác':
      return 'Chỉnh sửa'; // Có thể thay bằng nút hoặc link
    default:
      return hoaDon[column.replace(' ', '')] || 'N/A';
  }
};
